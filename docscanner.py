...
class DocumentImage:
    ...
    def _read(self) -> str:
        ...
        document_str: str = ''
        # <<< MODIFIED: Now captures the ocr_ready_image returned from _mark_region
        image, line_items_coordinates, ocr_ready_image = self._mark_region()

        # the invoice number usually lives in regions -1 to -3
        for regions in self._regions_list:
            for i in regions:
                try:
                    # <<< MODIFIED: Passes the clean ocr_ready_image to _read_text
                    t: str = self._read_text(ocr_ready_image, line_items_coordinates, -i).replace('\n', ' ')
                    self.logger.debug(f'Reading {self.filename} region: {i} result: {t}')
                    m: re.Match = self.regex.search(t)
                    document_str = m.group(1)
                    if 'statistics' in self.config:
    ...
    # <<< MODIFIED: Entire method refactored for better OCR results.
    def _mark_region(self) -> Tuple[Any, List[List[Tuple[Any, Any]]], Any]:
        """
        This method prepares a high-quality binary image and uses it to find text regions.
        :return: A tuple containing:
                 - The original image with regions drawn on it.
                 - A list of coordinates for each region.
                 - The clean, OCR-ready binary image.
        """
        image = cv2.imread(self.filename)
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

        # --- REVISED PREPROCESSING ---
        # Create ONE high-quality binary image for both OCR and contour detection.
        # No blur, larger block size (51), and a small constant (9).
        ocr_ready_image = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                                cv2.THRESH_BINARY, 51, 9)

        # --- REVISED CONTOUR FINDING ---
        # To find contours, we still dilate, but first, we must invert the
        # ocr_ready_image because dilation works on white areas (text is black).
        inverted_for_dilation = cv2.bitwise_not(ocr_ready_image)
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (9, 9))
        dilate = cv2.dilate(inverted_for_dilation, kernel, iterations=4)
        ...
        output_image = image.copy()  # Draw on a copy to preserve the original
        for c in cnts:
            ...
            cv2.rectangle(output_image, (x, y), (x + w, y + h), color=(255, 0, 255), thickness=3)
            line_items_coordinates.append([(x, y), (x + w, y + h)])

        return output_image, line_items_coordinates, ocr_ready_image

    # <<< MODIFIED: Method simplified to work with the pre-processed image.
    def _read_text(self, ocr_ready_image: Any, line_items_coordinates: list, index: int) -> str:
        """
        Extracts text from a specified region of the pre-processed binary image.
        :param ocr_ready_image: The clean binary image from _mark_region.
        :param line_items_coordinates: List of all region coordinates.
        :param index: The index of the region to read.
        :return: The OCR'd text as a string.
        """
        # get co-ordinates to crop the image
        c = line_items_coordinates[index]

        # cropping image img = image[y0:y1, x0:x1]
        # Crop from the clean OCR-READY image, not the original color one.
        img_crop = ocr_ready_image[c[0][1]:c[1][1], c[0][0]:c[1][0]]

        # --- REMOVED ---
        # The image is already perfectly pre-processed. No need for another threshold.
        # ret, thresh1 = cv2.threshold(img, 120, 255, cv2.THRESH_BINARY)

        # pytesseract image to string to get results
        text = str(pytesseract.image_to_string(img_crop, config='--psm 6'))
        return text
